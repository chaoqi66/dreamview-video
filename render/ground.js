...
const  { loadTexture } = require('../util/models');
const gridGround = require('../assets/images/ten_center.png');
...
module.exports = class Ground {
  ...
  update(world, coordinates, scene) {
    ...
    if (this.mesh.type === 'grid') {
      ...
      const adc = world.autoDrivingCar;
      let heading = adc.heading;
      let position = coordinates.applyOffset({ x: adc.positionX, y: adc.positionY });
      if (STORE.options.showVirtualCloseLoop || STORE.options.showCloseLoop) {
        if (world.virtualCar) {
          heading = world.virtualCar.heading;
          position = coordinates.applyOffset({ x: world.virtualCar.positionX, y: world.virtualCar.positionY });
        }
      }
      this.mesh.position.set(position.x + 0, position.y, 0);

      // const positionXDistance = Math.abs(this.mesh.position.x - position.x);
      // const positionYDistance = Math.abs(this.mesh.position.y - position.y);
      // console.log('this.mesh.position = ', this.mesh.position);
      // console.log('positionXDistance = ', positionXDistance);
      // console.log('positionYDistance = ', positionYDistance);
      // if (positionXDistance > 200 || positionYDistance > 200) {
      //   this.mesh.position.set(position.x, position.y, 0);
      // }
